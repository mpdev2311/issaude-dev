...
export default {
    ...
    actions :{
        ...
        PROCEDURE_TYPE_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await ProcedureTypesService.deleteById(id)
                .then(async (response)=>{
        ...
        PROCEDURE_TYPE_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.procedureTypes)

            if(getters.procedureType.id > 0)
                await ProcedureTypesService.update(getters.procedureType)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await ProcedureTypesService.create(getters.procedureType)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/procedure_types')
                        await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Tipo de procedimento salvo com sucesso!')
