...
export default {
    ...
    actions :{        
        ...
        EMPLOYEES_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.employee)

            if(getters.employee.id > 0)
                await EmployeesService.update(getters.employee)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                            await dispatch('EMPLOYEES_STORE_LOAD_BY_ID', getters.employee.id)
                            router.push('/employees')
                            await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Cadastro de funcionário salva com sucesso!')
                        }
                    })
            else
                await EmployeesService.create(getters.employee)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/employees')
