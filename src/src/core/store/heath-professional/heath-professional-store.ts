...
export default {
    ...
    actions :{        
        ...
        HEALTH_PROFESSIONAL_STORE_LOAD : async ({ commit }) => {
            await commit('HEALTH_PROFESSIONAL_STORE_LOAD', await HealthProfessionalService.getAll())
        },

        HEALTH_PROFESSIONAL_STORE_SET_BLANK : async ({ commit }, healthProfessional : HealthProfessional) =>{
            
            await commit('HEALTH_PROFESSIONAL_STORE_LOAD_BY_ID', healthProfessional)
        },

        HEALTH_PROFESSIONAL_PROFESSIONA_STORE_LOAD_GET : async ({ commit }, healthProfessional : HealthProfessional) => {  
        ...
        HEALTH_PROFESSIONAL_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await HealthProfessionalService.deleteById(id)
                .then(async (response)=>{
        ...
        HEALTH_PROFESSIONAL_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.healthProfessional)

            if(getters.healthProfessional.id > 0)
                await HealthProfessionalService.update(getters.healthProfessional)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await HealthProfessionalService.create(getters.healthProfessional)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/health_professionals')
                        await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Health Professional salva com sucesso!')
