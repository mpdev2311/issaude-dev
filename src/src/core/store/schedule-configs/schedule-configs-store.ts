...
export default {
    ...
    actions :{
        ...
        SCHEDULE_CONFIGS_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await ScheduleConfigService.deleteById(id)
                .then(async (response)=>{
        ...
        SCHEDULE_CONFIGS_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.scheduleConfig)

            if(getters.scheduleConfig.id > 0)
                await ScheduleConfigService.update(getters.scheduleConfig)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await ScheduleConfigService.create(getters.scheduleConfig)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/schedule_configs')
                        await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Cadastro agenda salvo com sucesso!')
