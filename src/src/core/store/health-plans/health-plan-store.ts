import HealthPlanService from "../../services/health-plans/health-plan-service"
import { HealthPlan } from "../../domain/health-plans/health-plan-model";
...
export default {
    ...
    actions :{
        ...
        HEALTH_PLAN_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await HealthPlanService.deleteById(id)
                .then(async (response)=>{
        ...
        HEALTH_PLAN_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.healthPlan)

            if(getters.healthPlan.id > 0)
                await HealthPlanService.update(getters.healthPlan)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await HealthPlanService.create(getters.healthPlan)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/plans')
