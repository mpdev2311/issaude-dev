    ...
    actions :{
       ...
       PRESENTATION_STORE_LOAD_BY_ID : async ({ commit }, id : number) => {
        ...
        PRESENTATION_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await PresentationService.deleteById(id)
                .then(async (response)=>{
        ...
        PRESENTATION_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.presentation)

            if(getters.presentation.id > 0)
                await PresentationService.update(getters.presentation)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await PresentationService.create(getters.presentation)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/Presentation')
