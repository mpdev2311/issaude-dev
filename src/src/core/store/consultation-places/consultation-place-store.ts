...
export default {
    ...
    actions :{
        ...
        CONSULTATION_PLACE_STORE_SET_BLANK : async ({ commit }, consultationPlace : ConsultationPlace) =>{
        ...
        CONSULTATION_PLACE_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await ConsultationPlacesService.deleteById(id)
                .then(async (response)=>{
        ...
        CONSULTATION_PLACE_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.consultationPlace)

            if(getters.consultationPlace.id > 0)
                await ConsultationPlacesService.update(getters.consultationPlace)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await ConsultationPlacesService.create(getters.consultationPlace)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/consultation_places')
                        await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Consultation Place salva com sucesso!')
