import AccountInfoService from "../../../core/services/accounts-infos/account-info-service"
...
export default {
    ...
    actions :{
        ...
        ACCOUNT_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await AccountInfoService.deleteById(id)
                .then(async (response)=>{
        ...
        ACCOUNT_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.accountInfo)

            if(getters.accountInfo.id > 0)
                await AccountInfoService.update(getters.accountInfo)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await AccountInfoService.create(getters.accountInfo)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                        router.push('/accountinfo')
                        await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Account info salva com sucesso!')
