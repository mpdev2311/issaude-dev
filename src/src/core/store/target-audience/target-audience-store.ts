...
export default {
    ...
    actions :{
        ...
        TARGET_AUDIENCE_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await TargetAudienceService.deleteById(id)
                .then(async (response)=>{
        ...
        TARGET_AUDIENCE_STORE_SAVE : async ({ getters, dispatch }) => {
            ...
            console.log(getters.targetAudience)

            if(getters.targetAudience.id > 0)
                await TargetAudienceService.update(getters.targetAudience)
                    .then(async (response)=>{
                        if(response.status === 200 || response.status === 201){
                ...
                await TargetAudienceService.create(getters.targetAudience)
                .then(async (response)=>{
                    if(response.status === 200 || response.status === 201){
                       // router.push('/target-audience')
                       // await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Target audience salva com sucesso!')
