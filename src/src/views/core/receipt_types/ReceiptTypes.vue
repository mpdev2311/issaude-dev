<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Tipo de Recebimento</h3>
    ...
    <div class="pt-1">
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6 space-y-1">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input disabled type="text" name="first_name" id="first_name" autocomplete="given-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="receiptType.id">
          </div>
        </div>
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input type="text" name="last_name" id="last_name" autocomplete="family-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="receiptType.nome">
          </div>
        </div>
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input type="number" name="last_name" id="last_name" autocomplete="family-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="receiptType.desconto">
          </div>
        </div>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const receiptType = ref<ReceiptType>({
  ...
  store.dispatch('RECEIPT_TYPE_STORE_SET_BLANK', receiptType.value)

  onMounted(async () => {
        if(router.currentRoute.value.params.id !== '0')
          await store.dispatch('RECEIPT_TYPE_STORE_LOAD_BY_ID', router.currentRoute.value.params.id)
        else 
          await store.dispatch('RECEIPT_TYPE_STORE_SET_BLANK', receiptType.value)
    });
    ...
    const onSave = async () => {
      await store.dispatch('RECEIPT_TYPE_STORE_SAVE')
    };
