    ...
    <div class="pt-1">
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6 space-y-1">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input type="text" name="first_name" id="first_name" autocomplete="given-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="user.nome">
          </div>
        </div>
...
export default defineComponent({
  ...
  onMounted(async () => {
        if(router.currentRoute.value.params.id !== '0')
          ...
          await store.dispatch('USER_STORE_SET_BLANK', user.value)
    });
