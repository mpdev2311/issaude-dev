      ...
      <div class="mt-4 flex flex-col sm:flex-row">
         <div class="flex">
            ...
            <div class="block relative mt-4 sm:mt-0">
               ...
               <input @change="onFilter" placeholder="" v-model="protocolTicket" class="appearance-none  border border-gray-400 border-b block pl-8 pr-6 py-2 w-full bg-white text-sm placeholder-gray-400 text-gray-700 focus:bg-white focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
            </div>
         </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('UNIT_STORE_LOAD')
    })

    const onEdit = (id) => {
       router.push(`/unitmeasure/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('UNIT_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () =>{
      router.push('/unitmeasure/0')
    }
