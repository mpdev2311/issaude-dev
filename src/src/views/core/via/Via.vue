<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Via</h3>
    <div class="mt-8">
      <div class="mt-4">
        ...
        <form class="space-y-4 divide-y divide-gray-200">
          <div class="space-y-8 divide-y divide-gray-200">
            <div class="pt-2">
              <div class="mt-6 grid grid-cols-2 gap-y-6 gap-x-4 sm:grid-cols-6 space-y-1">
                <div class="sm:col-span-3">
                  <label for="first_name" class="block text-sm font-medium text-gray-700">
                  Registro
                  </label>
                  <div class="mt-1">
                    <input disabled type="text" name="id" id="id" autocomplete="given-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="via.id">
                  </div>
                </div>
                <div class="sm:col-span-3">
                  ...
                  <div class="mt-1">
                    <input type="text" name="nome" id="nome" autocomplete="family-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="via.nome">
                  </div>
                </div>
              </div>
            </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    store.dispatch('VIA_STORE_SET_BLANK', via.value)

    onMounted(async () => {
        if(router.currentRoute.value.params.id !== '0')
          await store.dispatch('VIA_STORE_LOAD_BY_ID', router.currentRoute.value.params.id)
        else 
          await store.dispatch('VIA_STORE_SET_BLANK', via.value)
    });
    ...
    const onSave = async () => {
      await store.dispatch('VIA_STORE_SAVE')
    };
