<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Posologia</h3>
    ...
    <div class="pt-1">
      ...
      <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6 space-y-1">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input type="text" name="first_name" id="first_name" autocomplete="given-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="dosage.nome">
          </div>
        </div>
        <!-- 
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <select v-model="dosage.id_user"  id="tipo" name="tipo" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
           <select v-model="dosage.id_corp"  id="corporation" name="corporation" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        ...
        <div class="sm:col-span-3">
          ...
          <div class="mt-1">
            <input type="text" name="last_name" id="last_name" autocomplete="family-name" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md" v-model="dosage.quant">
          </div>
        </div>
...
export default defineComponent({
  ...
  store.dispatch('DOSAGE_STORE_SET_BLANK', dosage.value)

  onMounted(async () => {
        if(router.currentRoute.value.params.id !== '0')
          await store.dispatch('DOSAGE_STORE_LOAD_BY_ID', router.currentRoute.value.params.id)
        else 
          await store.dispatch('DOSAGE_STORE_SET_BLANK', dosage.value)
    });
    ...
    const onSave = async () => {
      await store.dispatch('DOSAGE_STORE_SAVE')
    };
