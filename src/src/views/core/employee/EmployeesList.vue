<template>
  <div>
    <NotificationAlert />
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Funcionários</h3>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('EMPLOYEES_STORE_LOAD')
    })
    const onEdit = (id) => {
       router.push(`/employees/${id}`)
    }
    const onDelete = async (i) => {
      await store.dispatch('EMPLOYEES_STORE_DELETE_BY_ID', i.id)
    }
    const onCreate = () =>{
      router.push('/employees/0')
    }
    return {
      onCreate,
      onEdit,
