  ...
  <div class="flex justify-center items-center h-screen bg-gray-200 px-6">
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
      ...
      <form class="mt-4 " @submit.prevent="login">
        <h2 class="mt-2 bg-white px-6 pt-12 pb-10 text-center text-2xl font-display text-gray-900 sm:-mx-6">
          Selecione uma impressora
        </h2>
         <ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
            <li v-for="(i, index) in printers" :key="index"
            class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
              ...
              <div class="ml-4 flex-shrink-0">
                 <label class="inline-flex items-center">
                  <input type="radio" @change="change(i)" v-model="i.selected" class="form-radio" name="selected" :value="i.selected">
                </label>
              </div>
            </li>
          </ul>
