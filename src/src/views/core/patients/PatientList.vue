<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Pacientes</h3>
    ...
    <div class="mt-4 flex flex-col sm:flex-row">
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          ...
          <!-- show message -->
              <div v-if="showMessage" class="flex justify-center px-4 py-3 bg-gray-50 text-right sm:px-6 bg-green-200">
          ...
          <!-- show message -->
                ...
                <div
               class="px-5 py-5 bg-white border-t  xs:flex-row items-center xs:justify-between "
               >
                <FullPagination v-bind:paginator="paginator" v-bind:onSelect="onPaginate"  />
            </div>
...
export default defineComponent({
  ...
  setup(_) {
    ...
    onMounted(async () => {
      await store.dispatch('PATIENTS_STORE_INIT')
       await store.dispatch('PATIENTS_STORE_FIND_ALL')
    ...
    const onDelete = async (i) => {
      await store.dispatch('PATIENTS_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () =>{
      router.push('/patients/0')
    }
