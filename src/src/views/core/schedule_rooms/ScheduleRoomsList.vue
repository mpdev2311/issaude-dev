<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Agenda e Salas</h3>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onEdit = (id) => {
       router.push(`/schedule_rooms/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('SCHEDULE_NAME_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () =>{
      router.push('/schedule_rooms/0')
    }
