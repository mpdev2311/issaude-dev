...
const value = computed(() => {
    ...
    if (props.options.every(op => typeof op === 'string')) {
      ...
      if (props.label) {
        let search = props.options.find(op => op[props.valueProp] == props.modelValue)
          ? props.options.find(op => op[props.valueProp] == props.modelValue)[props.label] : props.modelValue
        return search
      }
    }
  }
  if (typeof props.options === 'object' && !Array.isArray(props.options) && props.label) {
    //@ts-ignore
    return Object.keys(props.options).map(o => props.options[o]).find(od => od[props.valueProp] == props.modelValue)[props.label] || props.modelValue
  }
  ...
  <div ref="comboboxElement" class="combobox-body">
    ...
    <div class="relative mt-1">
      <input
        ...
        id="input_combobox"
        type="text"
        :class="['w-full rounded-md border border-gray-300 bg-white py-2 pl-3 pr-12 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm', combobox.isOpen ? 'ring-2 ring-indigo-500' : '']"
        role="combobox"
      ...
      <ul
        class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
        id="options-combobox"
        ref="lsoptions"
        role="listbox"
      >
        <li
          ...
          :class="['relative focus:text-white focus:outline-none focus:bg-blue-700 cursor-pointer hover:text-white hover:bg-blue-700 select-none py-2 pl-3 pr-9', op[props.valueProp] == modelValue ? 'bg-indigo-600' : 'text-gray-900']"
          :id="`option-${op[props.valueProp]}`"
          role="option"
          tabindex="-1"
          @keypress="() => { input_combobox.focus(), input_combobox.select() }"
          @keydown.arrow-down.prevent="combobox.arrowDown"
          @keydown.arrow-up.prevent="combobox.arrowUp"
        >
          <span
            :class="['block truncate', op[props.valueProp] == modelValue ? 'font-semibold text-white' : '']"
          >{{ op[props.label] }}</span>
          <span
            :class="[
