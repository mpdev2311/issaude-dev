  ...
  <div class="flex">
    ...
    >
      <div class="flex items-center justify-center mt-10">
       ...
       <div class="flex items-center justify-center mt-1">
        <div class="flex items-center">
          <span> 
              <br/><small class="text-green-600">Usuário conectado: </small><small class="text-white text-center"> {{user.nome.length > 10 ? user.nome.substr(0, 9) + " ..." : user.nome }}</small>
              <br/><small class="text-green-600">Corporação: </small><small class="text-white text-center"> {{corp.nome.length > 10 ? corp.nome.substr(0, 9) + " ..." : corp.nome }}</small>
            </span>
        </div>
      </div>
    ...
    <div class="w-full mt-15 mt-4 md:w-9/9 mx-auto ">
         <div class="shadow-md">
            ...
            <div 
              v-for:="i in modules" 
              class="tab w-full overflow-hidden border-l-4"
              >
               <input 
                  class="absolute opacity-0 " 
                  :id="[(`tab-multi-one-${i.nome}`)]" 
                  type="checkbox" name="tabs1"
                >
                <label 
                  class="block p-5 leading-normal cursor-pointer text-white" 
                  :for="[(`tab-multi-one-${i.nome}`)]"> {{i.nome}} </label
                >
               <div class="tab-content overflow-auto border-l-2 border-indigo-500 leading-normal">
                 <nav class="w-full">
                      <router-link
                        ...
                        :class="[$route.name === e.rota ? 'bg-gray-600 bg-opacity-25 text-gray-100 border-gray-100' : 'border-gray-900 text-gray-500 hover:bg-gray-600 hover:bg-opacity-25 hover:text-gray-100']"
                        :to="`${e.rota}`"
                      >
                        <svg
                          class="h-5 w-5"
...
import { useSidebar } from "../../../src/core/services/hooks/useSidebar";
import  AclProfileService  from '../../../src/core/services/acl/acl-profile/acl-profile-service'
