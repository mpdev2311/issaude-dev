                  ...
                  <template v-for="(column, id) in columns" :key="'col' + column.key">
                    <slot name="header-column" :column="column">
                      <component
                        ...
                        :class="[
                          column.headerCellClass ? column.headerCellClass : id === 0 ? 'sticky top-0 z-10 border-b border-gray-300 bg-gray-50 bg-opacity-75 py-2.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8' : 'sticky top-0 z-10 hidden border-b border-gray-300 bg-gray-50 bg-opacity-75 px-3 py-2.5 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:table-cell',
                        ]"
                      />
                      <th
                        v-else
                        scope="col"
                        :class="id === 0 ? 'sticky top-0 z-10 border-b border-gray-300 bg-gray-50 bg-opacity-75 py-2.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8' : 'sticky top-0 z-10 hidden border-b border-gray-300 bg-gray-50 bg-opacity-75 px-3 py-2.5 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:table-cell'"
                      >{{ column.label }}</th>
                    </slot>
                  </template>
                </slot>
