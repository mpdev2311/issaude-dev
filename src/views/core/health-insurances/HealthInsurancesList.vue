<template>
  <div>
    <NotificationAlert />
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Convênios</h3>
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in healthInsurances" :key="i" class="hover:bg-gray-200">
          ...
          <div
            v-if="showMessage"
            class="flex justify-center px-4 py-3 bg-gray-50 text-right sm:px-6 bg-green-200"
          >
            {{ messageShow }}
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['healthInsurances']),
    ...mapGetters(['healthInsurances']),
  },
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('HEALTH_INSURANCES_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/health_insurances/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('HEALTH_INSURANCES_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/health_insurances/0')
    }
