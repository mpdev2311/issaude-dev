<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Conta-Corrente</h3>
                  ...
                  <div class="sm:col-span-3">
                    ...
                    <div class="mt-1">
                      <input
                        ...
                        autocomplete="family-name"
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md"
                        v-model="accountInfo.data_abertura"
                      />
                    </div>
                  </div>
                  ...
                  <div class="sm:col-span-3">
                    ...
                    <div class="mt-1">
                      <input
                        ...
                        autocomplete="family-name"
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-200 rounded-md"
                        v-model="accountInfo.saldo_abertura"
                      />
                    </div>
                  </div>
...
import { AccountInfo } from '@core/domain/accountinfo/account-info.model'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    store.dispatch('ACCOUNT_STORE_SET_BLANK', accountInfo.value)

    onMounted(async () => {
      if (router.currentRoute.value.params.id !== '0')
        await store.dispatch('ACCOUNT_STORE_LOAD_BY_ID', router.currentRoute.value.params.id)
      else await store.dispatch('ACCOUNT_STORE_SET_BLANK', accountInfo.value)
    })
    ...
    const onSave = async () => {
      await store.dispatch('ACCOUNT_STORE_SAVE')
    }
