      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in accountInfos" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['accountInfos']),
    ...mapGetters(['accountInfos']),
  },
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('ACCOUNT_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/accountinfo/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('ACCOUNT_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/accountinfo/0')
    }
