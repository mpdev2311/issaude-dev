<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Tipos de Recebimento</h3>
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in receiptTypes" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['receiptTypes']),
    ...mapGetters(['receiptTypes']),
  },
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('RECEIPT_TYPE_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/receipt_types/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('RECEIPT_TYPE_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/receipt_types/0')
    }
