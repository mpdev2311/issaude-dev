      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in dosages" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('DOSAGE_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/dosages/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('DOSAGE_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/dosages/0')
    }
