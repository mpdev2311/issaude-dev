<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Tipos de Procedimento</h3>
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in procedureTypes" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('PROCEDURE_TYPE_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/procedure_types/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('PROCEDURE_TYPE_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/procedure_types/0')
    }
