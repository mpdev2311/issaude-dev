...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const getAddressData = async () => {
      ...
      await axios.get(`https://viacep.com.br/ws/${patient.value.cep}/json/`).then((response) => {
        let data = response.data
        patient.value.bairro = data.bairro
        patient.value.cidade = data.localidade
        patient.value.endereco = data.logradouro
        patient.value.uf = data.uf
        patient.value.cod_ibge = data.ibge
        const estado = props.estados.find((state) => state.sigla === data.uf)
        if (estado) {
