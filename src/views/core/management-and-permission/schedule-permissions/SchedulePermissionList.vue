<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Acesso agenda de consulta</h3>
          ...
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr class="hover:bg-gray-200">
                <td class="py-4 px-6 border-b text-gray-700 text-lg">{{}}</td>
                <td class="py-4 px-6 border-b text-gray-700 text-lg">{{}}</td>
                <td class="py-4 px-6 border-b text-gray-500">{{}}</td>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/schedule_permission/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('STORE_DELETE_BY_ID', i.id)
    }
