      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in consultationPlaces" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['consultationPlaces']),
    ...mapGetters(['consultationPlaces']),
  },
  ...
  setup(props) {
    ...
    const onEdit = (id) => {
      router.push(`/consultation_places/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('CONSULTATION_PLACE_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/consultation_places/0')
    }
