...
import {LocalAttendances} from "../../domain/local-attendance/local-attendance-model";
import LocalAttendanceService from "../../services/local-attendance/local-attendance-service";
...
export default {
    ...
    actions :{
        ...
        LOCAL_ATTENDANCE_STORE_DELETE_BY_ID : async ({ commit, dispatch }, id : number) => {
            await LocalAttendanceService.deleteById(id )
                .then(async (response)=>{
        ...
        LOCAL_ATTENDANCE_STORE_SAVE : async ({ getters, dispatch }) => {
            if(getters.localAttendance.id > 0)
                await LocalAttendanceService.update(getters.localAttendance)
                    .then(async (response)=>{
                        if (IsSuccessfulStatusCode(response.status)) {
                ...
                await LocalAttendanceService.create(getters.localAttendance)
                    .then(async (response)=>{
                        if (IsSuccessfulStatusCode(response.status)) {
                            router.push('/local-attendanceList')
