import AclGroupService from './acl-group-service'
import AclGroup from './acl-group-model'
...
export default {
  ...
  actions: {
    ...
    ACL_GROUP_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await AclGroupService.deleteById(id).then(async (response) => {
    ...
    ACL_GROUP_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.aclGroups.id > 0)
        await AclGroupService.update(getters.aclGroups).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await AclGroupService.create(getters.aclGroups).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
