  ...
  actions: {
    ACL_PROFILE_APPLY_ROUTE: async ({ commit, getters, state, dispatch }, route: any) => {
      if (getters.aclProfile.permissions.find((c) => c.rota.nome === route.rota.nome)) {
        await dispatch(
          'MESSAGE_STORE_ERROR_PUSH',
          `Já existe uma permissão para a rota ${route.rota.nome}!`
        )
        return
      }
    ...
    ACL_PROFILE_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await AclProfileService.deleteById(id).then(async (response) => {
    ...
    ACL_PROFILE_STORE_SAVE: async ({ getters, dispatch, state }) => {
      ...
      if (getters.aclProfile.id_perfil > 0)
        await AclProfileService.update(getters.aclProfile).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await AclProfileService.create(getters.aclProfile).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
