import AclModuleRouteService from '@domain/acl/acl-modules/acl-module-service'
...
export default {
  ...
  actions: {
    ...
    ACL_MODULE_ROUTE_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await AclModuleRouteService.deleteById(id).then(async (response) => {
    ...
    ACL_MODULE_ROUTE_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.aclModuleRoutes.id > 0)
        await AclModuleRouteService.update(getters.aclModuleRoutes).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await AclModuleRouteService.create(getters.aclModuleRoutes).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
