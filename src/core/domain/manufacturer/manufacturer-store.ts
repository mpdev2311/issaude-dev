...
export default {
  ...
  actions: {
    ...
    MANUFACTURERS_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await ManufacturersService.deleteById(id).then(async (response) => {
    ...
    MANUFACTURERS_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.manufacturer.id > 0)
        await ManufacturersService.update(getters.manufacturer).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            await dispatch('MANUFACTURERS_STORE_LOAD_BY_ID', getters.manufacturer.id)
            router.push('/manufacturers')
            await dispatch(
              'MESSAGE_STORE_SUCCESS_PUSH',
              'Cadastro de fabricantes salvo com sucesso!'
            )
          }
        })
      else
        await ManufacturersService.create(getters.manufacturer).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/manufacturers')
            await dispatch(
              'MESSAGE_STORE_SUCCESS_PUSH',
