...
export default {
  ...
  actions: {
    ...
    TABLE_NAMES_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await TableNameService.deleteById(id).then(async (response) => {
    ...
    TABLE_NAMES_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.tableName.id > 0)
        await TableNameService.update(getters.tableName).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await TableNameService.create(getters.tableName).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/plans')
