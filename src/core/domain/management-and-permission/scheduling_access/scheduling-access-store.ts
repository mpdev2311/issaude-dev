...
export default {
  ...
  mutations: {
    ...
    SCHEDULING_ACCESS_STORE_LOAD_BY_ID: (state, payload) =>
      (state.schedulingAccessObject = payload),
  },
  ...
  actions: {
    ...
    SCHEDULING_ACCESS_STORE_SET_BLANK: async (
      { commit },
      schedulingAccessObject: SchedulingAccess
    ) => {
    ...
    SCHEDULING_ACCESS_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await SchedulingAccessService.deleteById(id).then(async (response) => {
    ...
    SCHEDULING_ACCESS_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.schedulingAccessObject.id > 0)
        await SchedulingAccessService.update(getters.schedulingAccessObject).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
              await dispatch(
                'SCHEDULING_ACCESS_STORE_LOAD_BY_ID',
                getters.schedulingAccessObject.id
              )
        ...
        await SchedulingAccessService.create(getters.schedulingAccessObject).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
