...
export default {
  ...
  mutations: {
    ...
    APPOINTMENT_SCHEDULE_STORE_LOAD_BY_ID: (state, payload) =>
  ...
  actions: {
    ...
    APPOINTMENT_SCHEDULE_STORE_SET_BLANK: async (
      { commit },
    ...
    APPOINTMENT_SCHEDULE_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await AppointmentScheduleService.deleteById(id).then(async (response) => {
    ...
    APPOINTMENT_SCHEDULE_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.appointmentSchedule.id > 0)
        await AppointmentScheduleService.update(getters.appointmentSchedule).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
              await dispatch(
                'APPOINTMENT_SCHEDULE_STORE_LOAD_BY_ID',
                getters.appointmentSchedule.id
              )
        ...
        await AppointmentScheduleService.create(getters.appointmentSchedule).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
