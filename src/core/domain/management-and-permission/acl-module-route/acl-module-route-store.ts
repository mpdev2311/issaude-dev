import AclModuleRoutesService from './acl-module-route-service'
import { AclModuleRoute } from './acl-module-route-model'
...
export default {
  ...
  actions: {
    ...
    ACL_MODULE_ROUTE_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await AclModuleRoutesService.deleteById(id).then(async (response) => {
    ...
    ACL_MODULE_ROUTE_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.aclModuleRoute.id > 0)
        await AclModuleRoutesService.update(getters.aclModuleRoute).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await AclModuleRoutesService.create(getters.aclModuleRoute).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
