import GuideManagementService from '@domain/guide-management/guide-management-service'
...
export default {
  ...
  actions: {
    ...
    GUIDE_MANAGEMENT_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await GuideManagementService.deleteById(id).then(async (response) => {
    ...
    GUIDE_MANAGEMENT_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.guideManagement.id > 0)
        await GuideManagementService.update(getters.guideManagement).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await GuideManagementService.create(getters.guideManagement).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/guide-managements')
