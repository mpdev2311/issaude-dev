...
import FavoriteBanksService from './favorite-bank-service'
...
export default {
  ...
  actions: {
    ...
    FAVORITE_BANKS_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await FavoriteBanksService.deleteById(id).then(async (response) => {
    ...
    FAVORITE_BANKS_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.favoriteBank.id > 0)
        await FavoriteBanksService.update(getters.favoriteBank).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await FavoriteBanksService.create(getters.favoriteBank).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/favorite-banks')
            await dispatch('MESSAGE_STORE_SUCCESS_PUSH', 'Cadastro de convênios salva com sucesso!')
          }
