import TypeGuideService from './type-guides-service'
...
export default {
  ...
  actions: {
    ...
    TYPE_GUIDES_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await TypeGuideService.deleteById(id).then(async (response) => {
    ...
    TYPE_GUIDES_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.typeGuide.id > 0)
        await TypeGuideService.update(getters.typeGuide).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await TypeGuideService.create(getters.typeGuide).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/plans')
