...
import { ProductClassification } from './product-classification-model'
...
export default {
  ...
  mutations: {
    ...
    PRODUCT_CLASSIFICATION_STORE_LOAD_BY_ID: (state, payload) =>
      (state.productClassification = payload),
  },
  ...
  actions: {
    ...
    PRODUCT_CLASSIFICATION_STORE_SET_BLANK: async (
      { commit },
    ...
    PRODUCT_CLASSIFICATION_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await ProductClassificationService.deleteById(id).then(async (response) => {
    ...
    PRODUCT_CLASSIFICATION_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.productClassification.id > 0)
        await ProductClassificationService.update(getters.productClassification).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
              await dispatch(
                'PRODUCT_CLASSIFICATION_STORE_LOAD_BY_ID',
                getters.productClassification.id
              )
              router.push('/product-classification')
              await dispatch(
                'MESSAGE_STORE_SUCCESS_PUSH',
                'Product Classification salva com sucesso!'
              )
            }
          }
        )
      else
        await ProductClassificationService.create(getters.productClassification).then(
          async (response) => {
            if (IsSuccessfulStatusCode(response.status)) {
              router.push('/product-classification')
              await dispatch(
