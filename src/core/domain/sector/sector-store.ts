...
export default {
  ...
  actions: {
    ...
    SECTORS_STORE_DELETE_BY_ID: async ({ commit, dispatch }, id: number) => {
      await SectorsService.deleteById(id).then(async (response) => {
    ...
    SECTORS_STORE_SAVE: async ({ getters, dispatch }) => {
      if (getters.sector.id > 0)
        await SectorsService.update(getters.sector).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
        ...
        await SectorsService.create(getters.sector).then(async (response) => {
          if (IsSuccessfulStatusCode(response.status)) {
            router.push('/sectors')
            await dispatch(
              'MESSAGE_STORE_SUCCESS_PUSH',
