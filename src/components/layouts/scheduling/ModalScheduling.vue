...
const localAccess = computed(() => {
  let _localAccess = store.getters.localAccess
  if (_localAccess.hasOwnProperty('content')) {
    return _localAccess.content
  }
  return []
})
...
async function sendScheduling() {
  ...
  if (save.status === 200 || save.status === 201) {
    ...
    onCloseModal()
    generateSuccessAlert('Agendamento', 'Agendamento adicionado.')
    emit('updateSchedulings', date)
    return
  }
  generateErrorAlert('Agendamento', 'Erro ao adicionar agendamento!')
}
