...
function managementSelected(service: string) {
  switch (service.toLowerCase()) {
    case 'definir status':
      //@ts-ignore
      if(refStatus.value) {refStatus.value.showModal()}
      break;
...
<template>
  <Modal title="Ordem de Atendimento" :modal-is-open="modalIsOpen" @on-close-modal="closeModal" sm hidden-footer title-center>
    <template #body>
      <div class="w-full h-full relative flex flex-col items-center justify-center">
        <button v-for="(management, id) in managements" :key="id" @click="managementSelected(management.servico)" class="w-7/12 my-0.5 py-4 px-4 border-2 border-gray-300 text-center rounded-md font-semibold text-base hover:bg-gray-300 hover:border-gray-400">
          {{ management.servico }}
        </button>
      </div>
    </template>
