...
<template>
  <Alert ref="refAlert"/>
  <Modal title="Definir Status" id="refStatus" ref="refStatus" :modal-is-open="modalIsOpen" @on-close-modal="closeModal"
    sm hidden-footer title-center>
    <template #body>
      <div class="w-full h-auto relative flex flex-col items-center justify-center">
        <button v-for="(status, id) in schedulingStatus" :key="id" @click="updateStatus(status.id)"
