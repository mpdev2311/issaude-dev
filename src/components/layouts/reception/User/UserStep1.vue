...
<template>
    ...
    <div class="relative grid sm:grid-cols-12 grid-cols-1 sm:gap-x-8 gap-x-2 gap-y-8 px-6 py-10">
      ...
      <div class="col-span-12 sm:col-span-4">
        ...
        <button
          ...
          v-else>Alterar Senha</button>
      </div>
      <div class="col-span-12 sm:col-span-4">
        <label class="block text-sm font-medium text-gray-700">CPF</label>
        <Input v-model="user.cpf" />
        <p :class="[validateCPF && 'invisible']" class="mt-1.5 text-sm text-red-600">Informe um CPF válido.</p>
      </div>
      <div class="col-span-12 sm:col-span-4">
      ...
      <div class="col-span-12 sm:col-span-4">
        <label class="block text-sm font-medium text-gray-700">Bloquear usuário após: {{user.bloquear_apos != 0 && user.bloquear_apos != '' ? user.bloquear_apos + ' tentativas' : ''}}</label>
  ...
  <Modal sm :modal-is-open="modalIsOpen" title="Alterar Senha" hidden-footer @on-close-modal="modalIsOpen = false">
    <template #body>
