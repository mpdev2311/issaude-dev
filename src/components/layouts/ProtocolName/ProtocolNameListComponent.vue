      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in protocolNames" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['protocolNames']),
    ...mapGetters(['protocolNames']),
  },
  ...
  setup(props) {
    ...
    const onEdit = (id) => {
      router.push(`/protocol_names/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('PROTOCOL_NAME_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/protocol_names/0')
    }
