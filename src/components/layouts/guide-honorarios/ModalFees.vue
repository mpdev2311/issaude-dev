<template>
 <!-- <Alert ref="refAlert"/> -->
  <Modal
    ...
    title="Procedimentos"
    :modal-is-open="modalIsOpen" @on-close-modal="onCloseModal" mg @save="onSave"
    >
    <template #body>
      <div class="mt-6 pl-1 mb-4 grid gap-4 gap-6">
        ...
        <div class="col-span-11 sm:col-span-6 m-1">
          ...
          ><!--model codigo_tiss-->
           <Combobox :options="state.products" label="descri" v-model="billsSadt.id_produto" @change-search="onSearchProduct" hidden-icon>
      ...
      <!-- <div class="mt-6 pl-1 grid grid-cols-12 gap-4 gap-6">
       
      </div> -->

      <div class="mt-4 pl-1 grid grid-cols-12 gap-4 gap-6 m-2">
        <div class="col-span-11 sm:col-span-6">
...
<script lang="ts">
import { defineComponent, ref, onMounted, watchEffect, defineProps, defineEmits, reactive } from 'vue'
import { useRouter } from 'vue-router'
import { mapGetters, mapMutations, mapState, useStore } from 'vuex'
import { key, store } from '../../../core/store/store'
import Modal from '../../modal/Modal.vue'
import { Bill } from '../../../core/domain/guide-sadt/bills-model'
import Combobox from '../../../components/combobox/Combobox.vue'
//import { ref, watchEffect, reactive, computed, onMounted } from 'vue'/
import { PAGINATION_SIZE } from '@core/constaints'
//import { Product } from '../../../core/domain/guide-management/guide-management-model'
import ProductService from "../../../core/services/product/product-service";
import { RequestProduct, Product } from '../../../core/domain/product/product-model';
...
export default defineComponent({
  ...
  setup(props, {emit}) {
   ...
   // store.dispatch('BILLS_STORE_SET_BLANK', billsSadt.value)
    ...
    //   console.log(save.status === 200 || save.status === 201);
 // if(save.status === 200 || save.status === 201) {
   // let date = billsSadt.data_agendamento
     onCloseModal()
    }
...
async function getProviders(nome: string) {
  ...
  await store.dispatch('SEARCH_PROVIDER', {
    ...
    const onSearchProduct= async (descricao: string) => {
     ...
     let requestProduct = new RequestProduct(0, PAGINATION_SIZE, store.getters.user.id_corp, descricao,10101012 ,122)
      const productResults = await ProductService.searchProduct(requestProduct)
