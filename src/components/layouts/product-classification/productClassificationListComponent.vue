<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro Classificação de Produtos</h3>
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in productClassifications" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  ...
  computed: {
    ...mapState(['productClassifications']),
    ...mapGetters(['productClassifications']),
  },
  ...
  setup(props) {
    ...
    onMounted(async () => {
      await store.dispatch('PRODUCT_CLASSIFICATION_STORE_LOAD')
    })

    const onEdit = (id) => {
      router.push(`/product-classification/${id}`)
    }

    const onDelete = async (i) => {
      await store.dispatch('PRODUCT_CLASSIFICATION_STORE_DELETE_BY_ID', i.id)
    }

    const onCreate = () => {
      router.push('/product-classification/0')
    }
