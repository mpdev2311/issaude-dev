<template>
  <div>
    <h3 class="text-gray-700 text-2xl font-semibold">Cadastro de Banco Favorito</h3>
      ...
      <div class="mt-6">
        <div class="bg-white shadow rounded-md overflow-hidden my-6">
          <table class="text-left w-full border-collapse">
            ...
            <tbody>
              <tr v-for="i in favoriteBanks" :key="i" class="hover:bg-gray-200">
...
export default defineComponent({
  components: {},
  computed: {
    ...mapState(['favoriteBanks']),
    ...mapGetters(['favoriteBanks']),
  },
  ...
  setup(props) {
    ...
    const onEdit = (id) => {
      router.push(`/favorite-banks/${id}`)
    }
    const onDelete = async (i) => {
      await store.dispatch('FAVORITE_BANKS_STORE_DELETE_BY_ID', i.id)
    }
    const onCreate = () => {
      router.push('/favorite-banks/0')
    }
    return {
      onCreate,
      onEdit,
