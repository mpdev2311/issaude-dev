...
import VEndStep from './VEndStep.vue'
...
import VGroupButton from '@components/base/button/VGroupButton.vue'
  ...
  <div class="w-auto h-auto bg-white">
      ...
      >
        <li v-for="(step, id) in props.steps" :key="id" class="relative md:flex-1 md:flex">
          <VStepPanel
            ...
            :id="step.id"
          />
          <VEndStep v-if="id !== steps.length - 1" />
        </li>
      </ol>
    </nav>
    ...
    >
      <div class="flex flex-row">
        ...
        <VButton color="white" @click.prevent="cancel()"> Cancelar </VButton>
        <VButton @click.prevent="save()"> Salvar </VButton>
      </div>
    </div>
  </div>
</template>
